...
工作区前端必须以中文可操作工作台为交付目标，布局参考 NoteGen 的顶栏、左侧模块导航、资源侧栏、中间编辑器和右侧辅助面板。布局元素必须承载真实业务操作，避免只呈现静态结构。

#### 顶栏

- 展示中文应用名、当前同步状态、当前用户入口和退出入口。
- 提供新建笔记、新建记录、新建文件夹、新建知识库、刷新同步等真实操作按钮。
- 快速记录入口按中文命名展示，包括文本、待办、链接、图片和文件，其中未完成能力使用明确的禁用状态或占位状态。

#### 左侧模块导航

- 模块入口使用中文命名：写作、记录、搜索、共享、AI、开发者、设置。
- 切换模块必须更新主工作区内容或右侧辅助面板内容。
- 当前模块必须有明确选中状态。

#### 左侧资源栏

- 写作模块展示知识库选择、文件夹树和笔记列表。
- 记录模块展示记录列表和新建记录入口。
- 共享资源必须展示共享来源、权限级别和只读标识。
- 空状态、加载状态和错误状态必须以中文展示。

#### 中间编辑区

- 笔记编辑区包含标题输入、Markdown 正文输入、保存按钮、草稿提示、同步状态、字数统计和版本信息。
- 记录编辑区包含记录标题、记录内容、保存入口和转为笔记的预留入口。
- 只读资源进入编辑区时，输入控件必须禁用，并展示中文只读原因。
